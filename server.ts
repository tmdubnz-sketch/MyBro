...
async function startServer() {
  ...
  wss.on('connection', (clientWs: WebSocket, req: http.IncomingMessage) => {
    ...
    const personaName = url.searchParams.get('persona') || 'Amo';
...
`;
      ...
      const systemInstruction = `${persona} You are a helpful AI assistant with a grounded New Zealand Māori persona. Keep answers very short, direct, and conversational. NEVER use Australian terms like "mate". Use slang and Te Reo Māori very sparingly (an occasional "bro" or "kia ora" is enough). Do not overdo the slang, phrases, or cultural terms. Treat the user with respect. ${pronunciationRules}`;
    ...
    geminiWs.on('message', (raw: Buffer) => {
      try {
        ...
        if (msg.error) {
          console.error(`[${sessionId}] Gemini API Error:`, msg.error);
          if (clientWs.readyState === WebSocket.OPEN) {
            clientWs.send(JSON.stringify({ type: 'error', message: msg.error.message || 'Gemini API Error' }));
          }
          return;
        }
